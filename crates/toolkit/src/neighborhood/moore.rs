use crate::{lattice::lattice2::{Lattice2, Lattice2Point}, types::{Lattice, Neighborhood, NeighborhoodBuilder}};
...
impl <S> Neighborhood for MooreNeighborhood<S> {
    type State = S;
    type Iter<'a> = std::slice::Iter<'a, S> where S: 'a;
...
impl <S: Clone + Default> NeighborhoodBuilder<Lattice2<S>> for MooreNeighborhoodBuilder {
    ...
    fn build_neighborhood(&self, point: &<Lattice2<S> as crate::prelude::Lattice>::Point, lattice: &Lattice2<S>) -> Self::Neighborhood {
